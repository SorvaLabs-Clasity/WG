...
export default function BranchList({ repo, defaultBranch }: BranchListProps) {
  ...
  const handleProtectClick = (branch: string) => {
    ...
    setProtectRules({ ...DEFAULT_PROTECTION });
  ...
  const handleProtect = () => {
    ...
    protectMutation.mutate(
      { branch, protection: protectRules },
      {
        onSuccess: () => {
  ...
  return (
    <>
      ...
      {/* PROTECT BRANCH MODAL */}
      {protectDialogOpen && protectTarget && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-[#24292f]/40 backdrop-blur-[3px] animate-fade-in" onClick={() => setProtectDialogOpen(false)}></div>
          <div className="bg-white rounded-[12px] shadow-modal border border-black/10 w-full max-w-[600px] relative z-10 animate-slide-up overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 border-b border-gh-border flex items-center justify-between bg-white pt-5 shrink-0">
              <h3 className="text-lg font-bold text-gray-900 tracking-tight">
                Protect Branch: <span className="font-mono bg-gray-100 px-1 rounded text-gh-blue">{protectTarget}</span>
              </h3>
              <button 
                onClick={() => setProtectDialogOpen(false)}
                className="w-8 h-8 rounded-md flex items-center justify-center text-gray-400 hover:text-gray-900 hover:bg-black/5 transition-colors absolute right-4 top-4"
              >
                <i className="ph ph-x text-lg"></i>
              </button>
            ...
            <div className="px-6 py-4 border-t border-gh-border bg-gray-50/50 flex items-center justify-end gap-3 rounded-b-[12px] shrink-0">
              ...
              >
                {protectMutation.isPending ? "Applying..." : "Apply Protection"}
              </button>
            </div>
          </div>
        </div>
