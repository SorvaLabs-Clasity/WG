      ...
      <div className="flex flex-wrap gap-1.5">
        {variables.map(v => (
          <button key={v.name} type="button" title={v.description}
            ...
            className="px-2 py-1 rounded-md text-xs font-mono bg-black/5 dark:bg-ink/10 text-gh-textBase dark:text-slate-200 hover:bg-ink hover:text-ink transition-colors">
            {`{{${v.name}}}`}
          </button>
        ))}
      </div>
