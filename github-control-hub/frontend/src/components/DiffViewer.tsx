      ...
      <table className="w-full border-collapse text-left">
        <tbody>
          {diffLines.map((line, i) => {
            let bgColor = "bg-white dark:bg-paper";
            let textColor = "text-gh-textBase dark:text-slate-200";
            let symbol = " ";
            
            if (line.type === "added") {
              bgColor = "bg-forest-wash dark:bg-forest-wash";
              textColor = "text-ink dark:text-slate-200";
              symbol = "+";
            } else if (line.type === "removed") {
              bgColor = "bg-crimson-wash dark:bg-crimson-wash";
              textColor = "text-ink dark:text-slate-200";
              symbol = "-";
            }
