...
function riskColor(level: string) {
  switch (level) {
    case "CRITICAL": return { bg: "bg-rose-500", text: "text-white", ring: "ring-rose-300 dark:ring-rose-700", light: "bg-rose-50 dark:bg-rose-950/50 text-rose-700 dark:text-rose-400 border-rose-200 dark:border-rose-800" };
    case "HIGH": return { bg: "bg-orange-400", text: "text-white", ring: "ring-orange-300 dark:ring-orange-700", light: "bg-orange-50 dark:bg-orange-950/50 text-orange-700 dark:text-orange-400 border-orange-200 dark:border-orange-800" };
    case "MEDIUM": return { bg: "bg-amber-400", text: "text-white", ring: "ring-amber-300 dark:ring-amber-700", light: "bg-amber-50 dark:bg-amber-950/50 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-800" };
    default: return { bg: "bg-emerald-400", text: "text-white", ring: "ring-emerald-300 dark:ring-emerald-700", light: "bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800" };
  }
}
...
export default function KnowledgeGraphPage() {
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-950 min-h-screen pt-14">
      ...
      <main className="max-w-[1400px] mx-auto px-4 sm:px-6 py-8 animate-fade-in">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
          <div className="flex items-start gap-5">
            <div className="w-14 h-14 rounded-2xl bg-slate-900 dark:bg-slate-800 text-white flex items-center justify-center shadow-lg shadow-slate-900/20 shrink-0">
              <i className="ph-fill ph-graph text-2xl"></i>
            </div>
            <div>
              <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight mb-1">Knowledge Map</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            <div key={s.label} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm p-5 flex items-center gap-4">
              <div className={`w-10 h-10 rounded-xl bg-${s.color}-50 dark:bg-${s.color}-950/50 text-${s.color}-600 dark:text-${s.color}-400 flex items-center justify-center shrink-0`}>
                <i className={`ph-fill ${s.icon} text-xl`}></i>
              </div>
              <div>
                <div className="text-2xl font-bold text-slate-900 dark:text-white font-mono">{s.count}</div>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1">
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                  <h2 className="font-semibold text-slate-900 dark:text-white text-sm flex items-center gap-2">
                    <i className="ph-fill ph-squares-four text-slate-400 dark:text-slate-500"></i>
                    Risk Heat Map
                  </h2>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Click a repository to explore its relationships</p>
                </div>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="pl-9 pr-3 py-2 text-sm border border-slate-200 dark:border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900/10 dark:focus:ring-slate-400/20 focus:border-slate-300 dark:focus:border-slate-500 w-full sm:w-56 dark:bg-slate-800 dark:text-slate-200 dark:placeholder-slate-500"
                  />
                </div>
              </div>
              ...
              {rankingLoading ? (
                <div className="p-12 flex justify-center">
                  <div className="animate-spin rounded-full h-8 w-8 border-2 border-slate-200 dark:border-slate-700 border-t-slate-600 dark:border-t-slate-300"></div>
                </div>
              ) : (
                <div className="p-5">
                  {/* Legend */}
                  <div className="flex items-center gap-4 mb-4 text-xs text-slate-500 dark:text-slate-400">
                    <span className="font-medium text-slate-700 dark:text-slate-300">Risk Level:</span>
                      ...
                      return (
                        <button
                          ...
                          className={`group relative rounded-xl p-3 text-left transition-all duration-200 border-2 ${
                            isSelected
                              ? `${rc.bg} ${rc.text} border-slate-900 dark:border-white shadow-lg scale-[1.03]`
                              : `bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-500 hover:shadow-md hover:-translate-y-0.5`
                          }`}
                        >
                          <div className={`w-2 h-2 rounded-full mb-2 ${isSelected ? 'bg-white/70' : rc.bg}`}></div>
                          <div className={`text-xs font-semibold truncate ${isSelected ? '' : 'text-slate-800 dark:text-slate-200'}`} title={r.name}>
                            {r.name}
                          </div>
                          <div className={`text-[10px] mt-1 font-mono ${isSelected ? 'opacity-80' : 'text-slate-400 dark:text-slate-500'}`}>
          ...
          <div className="w-full lg:w-[420px] shrink-0">
            {selectedRepo ? (
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm p-8 text-center">
                <div className="w-16 h-16 rounded-2xl bg-slate-50 dark:bg-slate-800 text-slate-300 dark:text-slate-600 flex items-center justify-center mx-auto mb-4">
                  <i className="ph-fill ph-cursor-click text-3xl"></i>
                </div>
                <h3 className="font-semibold text-slate-700 dark:text-slate-300 mb-1">Select a Repository</h3>
  ...
  const sections: { key: string; label: string; icon: string; color: string; items: any[]; render: (item: any) => React.ReactNode }[] = [
    {
      ...
      render: (b) => (
        <div className="flex items-center justify-between py-1.5">
          ...
          {b.protected && <span className="text-[10px] px-1.5 py-0.5 rounded bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800 font-medium shrink-0">Protected</span>}
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (t) => (
        <div className="flex items-center justify-between py-1.5">
          ...
          {t.permission && <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700 font-medium shrink-0">{t.permission}</span>}
        </div>
      ),
    },
    {
      ...
      render: (w) => (
        <div className="py-1.5">
          <span className="text-sm text-slate-700 dark:text-slate-300 font-mono">{w.name}</span>
        </div>
      ),
    },
    {
      ...
      render: (d) => (
        <div className="flex items-center justify-between py-1.5">
          <span className="text-sm text-slate-700 dark:text-slate-300 font-mono">{d.name}</span>
          {d.severity && (
            <span className={`text-[10px] px-1.5 py-0.5 rounded border font-bold uppercase shrink-0 ${
              d.severity === "critical" ? "bg-rose-50 dark:bg-rose-950/50 text-rose-700 dark:text-rose-400 border-rose-200 dark:border-rose-800"
              : d.severity === "high" ? "bg-orange-50 dark:bg-orange-950/50 text-orange-700 dark:text-orange-400 border-orange-200 dark:border-orange-800"
      ...
      <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700 bg-gradient-to-r from-slate-50 to-white dark:from-slate-800 dark:to-slate-900">
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="font-bold text-slate-900 dark:text-white truncate" title={repo}>{repo}</h3>
              <div className="flex items-center gap-2 mt-0.5">
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border uppercase ${rc.light}`}>{level}</span>
                {ranking && <span className="text-xs text-slate-400 dark:text-slate-500 font-mono">{ranking.score} pts</span>}
              </div>
            </div>
          </div>
          <button onClick={onClose} className="text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 transition-colors p-1 -mr-1">
        ...
        {!isLoading && (
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-100 dark:border-slate-700 px-3 py-2 text-center">
              <div className="text-lg font-bold text-slate-900 dark:text-white font-mono">{grouped.branches.length}</div>
              <div className="text-[10px] text-slate-500 dark:text-slate-400 font-medium">Branches</div>
            </div>
            <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-100 dark:border-slate-700 px-3 py-2 text-center">
              <div className="text-lg font-bold text-slate-900 dark:text-white font-mono">{grouped.collaborators.length}</div>
              <div className="text-[10px] text-slate-500 dark:text-slate-400 font-medium">People</div>
            </div>
            <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-100 dark:border-slate-700 px-3 py-2 text-center">
              <div className="text-lg font-bold text-slate-900 dark:text-white font-mono">{grouped.dependencies.length}</div>
              <div className="text-[10px] text-slate-500 dark:text-slate-400 font-medium">Vulns</div>
            </div>
          </div>
        )}
      </div>
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-700">
            ...
            {blastData && (blastData.workflows.length > 0 || blastData.teamsWithAccess.length > 0) && (
              <div className="px-5 py-4">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mb-3 leading-relaxed">
                  If <strong className="text-slate-700 dark:text-slate-300">{repo}</strong> is compromised, the following are at risk:
                </p>
                <div className="space-y-2">
                  {blastData.teamsWithAccess.length > 0 && (
                    <div className="flex flex-wrap gap-1.5">
                      {blastData.teamsWithAccess.map(t => (
                        <span key={t.name} className="text-[11px] px-2 py-1 rounded-lg bg-purple-50 dark:bg-purple-950/50 text-purple-700 dark:text-purple-400 border border-purple-200 dark:border-purple-800 font-medium">
                          {t.name} <span className="text-purple-400 dark:text-purple-500">({t.permission})</span>
                        </span>
                      ))}
                    </div>
                  )}
                  {blastData.workflows.length > 0 && (
                    <div className="flex flex-wrap gap-1.5">
                      {blastData.workflows.map(w => (
                        <span key={w} className="text-[11px] px-2 py-1 rounded-lg bg-teal-50 dark:bg-teal-950/50 text-teal-700 dark:text-teal-400 border border-teal-200 dark:border-teal-800 font-mono">
                          {w}
                        </span>
                      ))}
                    </div>
            ...
            {nodeData?.edges.length === 0 && (
              <div className="p-8 text-center text-slate-400 dark:text-slate-500">
                ...
                <p className="text-sm font-medium text-slate-500 dark:text-slate-400">No graph data for this repo</p>
        ...
        <div className="px-5 pb-3">
          ...
          {hasMore && showAll && (
            <button
              ...
              className="mt-2 text-xs text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300 font-medium"
            >
              Show less
            </button>
          )}
